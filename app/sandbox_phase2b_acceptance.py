...
async def run() -> Path:
    ...
    try:
        ...
        case = ResearchCase(
            case_id=run_id,
            query="Does AAPL's current price-to-value gap occur near the upper end of its recent external trading range, and should that change the next valuation investigation?",
            objective="Determine whether external 52-week trading-range evidence is needed before choosing the next valuation investigation.",
            valuation_context={"symbol": "AAPL", "selected_model": valuation.selected_model or ""},
            evidence=initial,
            max_iterations=3,
        )
        _write(
            directory,
            "01_initial_case.json",
            {
                "case": case.model_dump(mode="json"),
                "initial_evidence_ids": [item.evidence_id for item in case.evidence],
            },
        )
        ...
        if decision_1.action != "REQUEST_EVIDENCE":
            summary.update(
                {
                    "status": "STOPPED_NON_EVIDENCE_DECISION",
        ...
        _write(
            directory,
            "03_market_evidence.json",
            {
                "information_need": decision_1.request,
                "facts": [item.model_dump(mode="json") for item in added],
                "specialist_invocations": executor.invocation_count,
                "dispatch_latency_ms": (perf_counter() - dispatch_started) * 1000,
                "invariants": {
                    "external_authority_only": all(
                        item.numerical_authority == "external_source" for item in added
                    ),
                    "provenance_complete": all(bool(item.provenance) for item in added),
                },
            },
        )
        _write(
            directory,
            "04_updated_case.json",
            {
                "case": updated.model_dump(mode="json"),
                "added_evidence_ids": [item.evidence_id for item in added],
            },
        )
        decision_started = perf_counter()
        ...
        summary.update(
            {
                "status": "COMPLETED_TWO_DECISIONS",
                "total_latency_ms": (perf_counter() - started) * 1000,
