...
""" + json.dumps(
        {
            ...
            "executed_actions": [
                item.action.model_dump(mode="json") for item in case.executed_actions
            ],
...
async def run() -> Path:
    ...
    try:
        async with httpx.AsyncClient(
            base_url=os.environ["MINISTRAL_CONTROLLER_BASE_URL"].rstrip("/"), timeout=360.0
        ) as client:
        ...
        _write(
            directory,
            "semantic_closure_decision.json",
            {
                "raw_response": raw,
                "decision": semantic.model_dump(mode="json"),
        ...
        summary.update(
            {
                ...
                "final_uncertainty_ids": list(action.unresolved_uncertainty_ids),
            }
        )
