...
def _guidance(year: int = 2027, symbol: str = "ACME") -> ResearchEvidence:
    guidance = RevenueGuidance(
        symbol=symbol,
        metric="REVENUE",
        representation="ABSOLUTE_FY_RANGE",
        low=Decimal("110"),
        high=Decimal("130"),
        currency="USD",
    ...
    return ResearchEvidence(
        ...
        evidence={"claim": "Issuer FY revenue guidance", "sourcePath": "external.issuer.guidance"},
        source="Issuer earnings release",
        source_type="external",
        numerical_authority="external_source",
        provenance={"url": guidance.source_url, "published_at": "2026-08-01"},
...
def _action(**updates: object) -> ResearchAction:
    request = {
        ...
        "symbol": "ACME",
        "rationale": "Guidance warrants a bounded test.",
        "evidence_ids": ("guidance",),
        "analysis_mode": "EVIDENCE_GROUNDED_OVERRIDE",
        "assumption_application": "YEAR_1_REVENUE_GUIDANCE",
    }
    request.update(updates)
    return ResearchAction(
        action="REQUEST_VALUATION_ANALYSIS",
        reason="Apply revenue guidance.",
...
def _driver(growth: str) -> dict[str, object]:
    return {
        ...
        "ebitMargin": "0.2",
        "taxRate": "0.2",
        "depreciationAndAmortizationRate": "0.05",
        "capexRate": "0.1",
        "changeInNetWorkingCapitalRate": "0",
    }
...
def _template(year: int = 2027) -> dict[str, object]:
    ...
    return {
        ...
        "temporalContext": {
            "availability": "FISCAL_LABEL_AVAILABLE",
            "forecastPeriods": [{"ordinalYear": 1, "fiscalYear": year, "fiscalPeriod": "FY"}],
        },
        "templates": {
            "MATURE_TECH_PLATFORM": {
                "baseInputs": {
                    "startingRevenue": "100",
                    "waccRate": "0.1",
                    "terminalGrowthRate": "0.02",
                    "debtFinancingPolicy": {"type": "TARGET_DEBT_FINANCING_RATIO"},
                },
...
def _preview() -> dict[str, object]:
    ...
    return {
        ...
        "archetype": "MATURE_TECH_PLATFORM",
...
class _Client:
    ...
    async def preview_explicit_forecast(
        self, _symbol: str, *, archetype: str, scenarios=None
    ) -> dict[str, object]:
...
async def test_deterministically_applies_midpoint_to_base_year_one_only() -> None:
    ...
    case = ResearchCase(
        query="q", objective="o", valuation_context={"symbol": "ACME"}, evidence=(_guidance(),)
    )
    ...
    assert (
        drivers[1:]
        == _template()["templates"]["MATURE_TECH_PLATFORM"]["scenarios"]["BASE"][
            "explicitOperatingDrivers"
        ][1:]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (2028, _template(), "does not align"),
        (2027, {**_template(), "temporalContext": {"availability": "UNAVAILABLE"}}, "unavailable"),
    ],
)
async def test_rejects_temporal_mismatch_or_unavailable_context(
    guidance_year, template_value, message
) -> None:
    ...
    case = ResearchCase(
        ...
        evidence=(_guidance(guidance_year),),
    )
...
async def test_rejects_symbol_and_missing_guidance_provenance() -> None:
    ...
    mismatch = ResearchCase(
        query="q",
        objective="o",
        valuation_context={"symbol": "ACME"},
        evidence=(_guidance(symbol="OTHER"),),
    )
    with pytest.raises(IllegalResearchTransition, match="does not match"):
        await executor(mismatch, _action())
    no_guidance = ResearchEvidence(
        evidence_id="guidance",
        evidence={"claim": "x", "sourcePath": "external.x"},
        source="SEC",
        source_type="external",
        numerical_authority="external_source",
    )
