...
async def test_market_agent_converts_real_source_shape_to_provenance_complete_evidence() -> None:
    transport = httpx.MockTransport(
        lambda _request: httpx.Response(
            200,
            json={
                "chart": {"result": [{"meta": {"fiftyTwoWeekHigh": 222.5, "fiftyTwoWeekLow": 100}}]}
            },
        )
    )
    async with httpx.AsyncClient(transport=transport) as client:
        ...
        case = ResearchCase(
            query="What is the current market observation?",
            objective="Assess valuation",
            valuation_context={"symbol": "AAPL"},
        )
        action = ResearchAction(
            action="REQUEST_EVIDENCE",
            reason="Market data is absent",
            request="Obtain a current market observation.",
        )
...
async def test_market_executor_rejects_repeat_and_malformed_source_output() -> None:
    transport = httpx.MockTransport(
        lambda _request: httpx.Response(200, json={"chart": {"result": [{}]}})
    )
    async with httpx.AsyncClient(transport=transport) as client:
        agent = MarketInformationAgent(client)
        with pytest.raises(MarketInformationError, match="52-week range"):
            await agent.collect(
                MarketInformationRequest(symbol="AAPL", question="q", information_need="need")
            )
    ...
    existing = ResearchCase.model_validate(
        {
            **case.model_dump(),
            "evidence": [
                {
                    "evidence": {"claim": "prior", "sourcePath": "external.x"},
                    "source": "SEC",
        ...
        async def decide(self, case: ResearchCase) -> ResearchAction:
            ...
            if not case.evidence:
                return ResearchAction(
                    action="REQUEST_EVIDENCE",
                    reason="Market evidence is missing",
                    request="current market observation",
                )
            return ResearchAction(
                action="DELEGATE_SPECIALIST",
                reason="Updated evidence requires a fresh semantic decision",
            )

    transport = httpx.MockTransport(
        lambda _request: httpx.Response(
            200,
            json={"chart": {"result": [{"meta": {"fiftyTwoWeekHigh": 2, "fiftyTwoWeekLow": 1}}]}},
        )
    )
    async with httpx.AsyncClient(transport=transport) as client:
        ...
        graph = build_research_graph(
            controller,
            ResearchDispatcher(
                evidence_executor=MarketInformationEvidenceExecutor(MarketInformationAgent(client))
            ),
        )
        with pytest.raises(IllegalResearchTransition, match="no specialist"):
            await graph.ainvoke(
                ...
                config={"configurable": {"thread_id": "updated-evidence"}},
            )
