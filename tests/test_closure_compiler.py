...
def _case() -> ResearchCase:
    return ResearchCase(
        ...
        tracked_uncertainties=(
            ResearchUncertainty(uncertainty_id="nwc", description="NWC caveat", source_evidence_id="forecast"),
        ),
    )


def test_compiler_maps_model_selected_termination_without_selecting_uncertainties() -> None:
    action = compile_terminal_action(_case(), ClosureDecision(decision="TERMINATE", remaining_uncertainty_ids=("nwc",)))
...
def test_compiler_rejects_unknown_uncertainty() -> None:
    with pytest.raises(IllegalResearchTransition, match="tracked uncertainties"):
        compile_terminal_action(_case(), ClosureDecision(decision="TERMINATE", remaining_uncertainty_ids=("unknown",)))
